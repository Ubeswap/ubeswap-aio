...
export function usePools(poolKeys: [Currency | undefined, Currency | undefined][]): [PoolState, Pool | null][] {
    ...
    return useMemo(() => {
        return poolKeys.map((_key, index) => {
            ...
            const { result: globalState, loading: globalStateLoading, valid: globalStateValid } = _globalState0s[index];
            const { result: liquidity, loading: liquidityLoading, valid: liquidityValid } = _liquidities[index];
