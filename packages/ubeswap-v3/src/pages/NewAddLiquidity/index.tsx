...
export function NewAddLiquidityPage({
    ...
    const handlePopularPairSelection = useCallback((pair: [string, string]) => {
        const WMATIC = AlgebraConfig.CHAIN_PARAMS.wrappedNativeCurrency.address;
        history.push(
            `/add/${pair[0] === WMATIC ? AlgebraConfig.CHAIN_PARAMS.wrappedNativeCurrency.symbol : pair[0]}/${pair[1] === WMATIC ? AlgebraConfig.CHAIN_PARAMS.wrappedNativeCurrency.symbol : pair[1]}`
        );
        resetState();
    }, []);
